...
// from optional usage statistics; failed deliveries may retry. From 0.3.51 they
// go to app.dictivo.app; earlier versions keep using api.dictivo.app.
export const TRIAL_MILESTONE_LASTMOD = "2026-09-26";
export const TRIAL_MILESTONE_COPY = {
  en: {
    usage: "Usage statistics are off unless you enable them in Settings.",
    trial: "Separately from that setting, the Local trial reports two milestones to app.dictivo.app (api.dictivo.app in versions before 0.3.51): trial start and the first Local transcript successfully copied or pasted. Reports contain a hashed device identifier, platform, app version, trial-start time and a capability flag; the success report adds its time. Failed deliveries may retry. Neither report contains audio or transcript text.",
  ...
  de: {
    usage: "Nutzungsstatistiken bleiben aus, bis Sie sie in den Einstellungen aktivieren.",
    trial: "Unabhängig davon meldet der Local-Test zwei Meilensteine an app.dictivo.app (in Versionen vor 0.3.51 an api.dictivo.app): den Testbeginn und das erste erfolgreich kopierte oder eingefügte Local-Transkript. Die Meldungen enthalten eine gehashte Gerätekennung, Plattform, App-Version, Testbeginn und eine Kennzeichnung der unterstützten Erfassung; die Erfolgsmeldung ergänzt ihren Zeitpunkt. Fehlgeschlagene Übertragungen können wiederholt werden. Keine Meldung enthält Audio oder Transkripttext.",
  ...
  es: {
    ...
    trial: "Con independencia de ese ajuste, la prueba Local comunica dos hitos a app.dictivo.app (a api.dictivo.app en las versiones anteriores a la 0.3.51): el inicio y la primera transcripción Local copiada o pegada correctamente. Los informes incluyen un identificador de dispositivo con hash, plataforma, versión de la app, hora de inicio de la prueba y un indicador de compatibilidad con esta medición; el informe de éxito añade su hora. Los envíos fallidos pueden reintentarse. Ningún informe contiene audio ni texto transcrito.",
  ...
  nl: {
    ...
    trial: "Los van die instelling meldt de Local-proefperiode twee mijlpalen aan app.dictivo.app (in versies vóór 0.3.51 aan api.dictivo.app): de start en het eerste succesvol gekopieerde of geplakte Local-transcript. De meldingen bevatten een gehashte apparaat-ID, platform, appversie, starttijd van de proefperiode en een aanduiding dat deze meting wordt ondersteund; de succesmelding voegt het eigen tijdstip toe. Mislukte verzendingen kunnen opnieuw worden geprobeerd. Geen melding bevat audio of transcripttekst.",
  ...
  zh: {
    ...
    trial: "独立于该设置，Local 试用会向 app.dictivo.app（0.3.51 之前的版本为 api.dictivo.app）上报两个里程碑：开始试用，以及首次成功复制或粘贴 Local 转录结果。上报包含设备标识的哈希值、平台、应用版本、试用开始时间和是否支持该统计的标记；成功上报还包含成功时间。发送失败可能重试。两类上报均不包含音频或转录文本。",
    network: "应用还可能为更新检查、模型下载、许可证、账单、支持服务和可选 Cloud Fast 使用网络。这些产品操作与上传 Local 录音进行转写是不同的用途。",
  },
  ja: {
    usage: "使用統計は、設定で有効にするまで送信されません。",
    trial: "この設定とは別に、Local トライアルは開始時と、Local の文字起こし結果を初めて正常にコピーまたは貼り付けた時の2段階を app.dictivo.app（0.3.51 より前のバージョンでは api.dictivo.app）に報告します。報告にはハッシュ化されたデバイス識別子、プラットフォーム、アプリ版、試用開始時刻、この計測への対応フラグが含まれ、成功報告には成功時刻も含まれます。送信に失敗すると再試行する場合があります。どちらも音声や文字起こしテキストは含みません。",
  ...
  ko: {
    ...
    trial: "이 설정과 별도로 Local 체험은 시작 시점과 Local 전사 결과를 처음 성공적으로 복사하거나 붙여넣은 시점을 app.dictivo.app(0.3.51 이전 버전은 api.dictivo.app)에 보고합니다. 보고에는 해시 처리한 기기 식별자, 플랫폼, 앱 버전, 체험 시작 시각, 해당 측정 지원 여부가 포함되며 성공 보고에는 성공 시각도 추가됩니다. 전송이 실패하면 재시도할 수 있습니다. 어느 보고에도 오디오나 전사 텍스트는 포함되지 않습니다.",
