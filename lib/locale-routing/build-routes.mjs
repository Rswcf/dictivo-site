...
const matchesRule = (rule, path) => (rule.endsWith("/*") ? path.startsWith(rule.slice(0, -1)) : path === rule);

// Language menus link with ?lang=; every such URL must reach the Function so the choice is saved,
// including English-only pages that link to themselves.
export function languageChoicePaths(distDir) {
  const paths = new Set();
  for (const file of htmlFiles(distDir)) {
    for (const [, href] of readFileSync(file, "utf8").matchAll(/href="([^"]*[?&](?:amp;)?lang=[^"]*)"/g)) {
      const url = new URL(href.replaceAll("&amp;", "&"), SITE_ORIGIN);
...
export function buildRoutesConfig(routes, choicePaths = []) {
  const sections = [...new Set(Object.keys(routes.pages).map((path) => (path === "/" ? "/" : `/${path.split("/")[1]}/*`)))];
  const uncovered = choicePaths.filter((path) => !sections.some((rule) => matchesRule(rule, path)));
  const include = [...new Set([...sections, ...uncovered])].sort();
